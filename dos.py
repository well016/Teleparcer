...
# Нужно создать функцию (get_data) для получения данных(Цель, Время для одновременного запуска, Длительность в минутах)
                                                      #(https://kpfu.ru/,  13:00 ,                 120)
# Функция будет обращаться к серверу, чтобы получить данные, каждые 5 минут кратно минутам time
# Cервер будет в виде сайта (docs.google.com)
# Нужно еще создать функцию (post_data) которая единожды запустится во время атаки для определения количества компьютеров в сети
